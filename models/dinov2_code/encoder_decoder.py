...
class DepthEncoderDecoder(nn.Module):
    ...
    def inference(self, img, img_meta, rescale, size=None, mode="whole"):
        ...
        output = depth_pred
        # see if flip is enabled
        if "flip" not in img_meta:
            return output
    ...
    def vanilla_inference(self, img, img_meta, rescale, size=None):
        depth_pred = self.inference(img, img_meta, rescale, size=size)
    ...
    def forward_test(self, imgs, img_metas, **kwargs):
        ...
        Args:
            ...
            img_metas (List[List[dict]]): the outer list indicates test-time
                ...
                images in a batch.
        """
        # if img_metas is empty, call simple_test
        if len(img_metas) == 1:
